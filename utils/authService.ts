...
export const authService = {
  ...
  verifyEmail: async (token: string): Promise<ApiResponse<any>> => {
    try {
      const response = await api.post<ApiResponse<any>>('/api/v1/auth/verify-email', { token });
      return response.data;
    } catch (error) {
      return handleAuthError(error);
    }
  ...
  resendVerification: async (data: { email: string } | { phone: string }): Promise<ApiResponse<any>> => {
    try {
      const response = await api.post<ApiResponse<any>>('/api/v1/auth/resend-verification', data);
      return response.data;
    } catch (error) {
      return handleAuthError(error);
    }
  ...
  verifyPhone: async (otp: string): Promise<ApiResponse<any>> => {
    try {
      const response = await api.post<ApiResponse<any>>('/api/v1/auth/verify-phone', { otp });
      return response.data;
    } catch (error) {
      return handleAuthError(error);
    }
  ...
  socialAuth: async (socialData: {
    ...
    try {
      const response = await api.post<AuthResponse>('/api/v1/auth/social-auth', socialData);
      return response.data;
    } catch (error) {
      return handleAuthError(error);
    }
  ...
  sendPhoneOTP: async (phone: string): Promise<ApiResponse<any>> => {
    try {
      const response = await api.post<ApiResponse<any>>('/api/v1/auth/send-phone-otp', { phone });
      return response.data;
    } catch (error) {
      return handleAuthError(error);
    }
  ...
  verifyPhoneOTPAndReset: async (phone: string, otp: string): Promise<ApiResponse<any>> => {
    try {
      const response = await api.post<ApiResponse<any>>('/api/v1/auth/verify-phone-otp-reset', { 
        phone, 
        otp 
      });
      return response.data;
